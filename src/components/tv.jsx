    ...
    <section className="my-[2rem] p-[10px]">
      <div className="flex flex-col">
        ...
        <div className="my-[1rem]">
          <h1 className="md:text-[40px] md:leading-[45px] font-[600]">Trending Series</h1>
        </div>
            ...
            {currentMovies.map((m) => (
              <div key={m.id} className="flex flex-col shadow-lg h-[270px] items-center justify-center gap-[8px]">
                <div className="w-[150px] h-[150px]">
                  <img
                    src={`https://image.tmdb.org/tv/p/original/${m.poster_path}`}
                    alt="movie_pics"
                    className="w-full h-full"
                  />
                </div>
                <h1 className="w-[90%] text-center font-bold font-libre cursor-pointer"  ><Link to={`/tvdetails/${m.id}`}>{m.title}</Link></h1>
